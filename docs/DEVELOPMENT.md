...
- 冲突时 `replace` 保留项目 ID 并替换项目内容；替换产生的旧孤儿资产不自动删除，交给磁盘资产扫描和清理流程。
...
- 实际孤儿文件清理入口：`src/platform/web/browserPlatform.ts`

存储设置里的“图片资产引用”展示快照引用统计；“磁盘资产扫描”通过 platform bridge 实际枚举 `images/` 文件并统计字节数、孤儿文件和缺失引用。清理确认必须展示文件数、可释放空间和路径样例，且只能删除扫描时未被完整工作区数据引用的文件。
